...
class Imprisoner : Item(
    ...
    private fun isEmpty(stack: ItemStack): Boolean =
        stack.nbt?.get(PRISONER)?.asString() == null

    init {
        Registry.register(Registry.ITEM, AwesomeUtils.identifier(ID), this)
    }
    ...
    override fun appendTooltip(stack: ItemStack, world: World?, tooltip: MutableList<Text>, context: TooltipContext) {
        stack.nbt?.get(PRISONER)?.asString()?.let {
    ...
    private fun capture(player: PlayerEntity, hand: Hand, stack: ItemStack, entity: LivingEntity): Boolean {
        ...
        stack.nbt = entity.writeNbt(NbtCompound()).apply {
            putString(PRISONER, entity.type.registryEntry.registryKey().value.toString())
            remove("Pos")
        }
    ...
    private fun release(world: ServerWorld, player: PlayerEntity?, pos: BlockPos, stack: ItemStack): Boolean {
        val id = stack.nbt?.getString(PRISONER) ?: return false
