...
fun Recipe<*>.test(stacks: List<ItemStack>): Int {
    ...
    if (ingredients.any { ingredient -> inventory.stacks.none { ingredient.test(it) } }) return -2
    ingredients.forEach { ingredient ->
        val index = inventory.stacks.indexOfFirst { ingredient.test(it) }.takeIf { it >= 0 } ?: return -3
