...
class RecyclerBlock(
    settings: Settings,
) : AwesomeBlockWithEntity<RecyclerBlockEntity>(AwesomeUtils.identifier(Recycler.ID), settings) {
    ...
    override fun createBlockEntity(pos: BlockPos, state: BlockState): RecyclerBlockEntity =
    ...
    override fun onUse(state: BlockState, world: World, pos: BlockPos, player: PlayerEntity, hand: Hand, hit: BlockHitResult): ActionResult {
        if (!world.isClient) {
            val blockEntity = world.getBlockEntity(pos) as? RecyclerBlockEntity
            state.createScreenHandlerFactory(world, pos)?.let { screenHandlerFactory ->
                player.openHandledScreen(screenHandlerFactory)
                (player.currentScreenHandler as? RecyclerScreenHandler)?.let { screenHandler ->
